{-# OPTIONS --guardedness --safe -WnoPatternShadowsConstructor #-}
...
--    data Gram : (Lang → Lang) → Set₁ where
--        ∅ : Gram (λ _ _ → ⊥)
--        ε : Gram (λ _ w → w ≡ [])
--        ‵_ : (c : Tok) → Gram (λ _ w → w ≡ (c ∷ []))
--        _·_ : Dec A → Gram φ → Gram (λ ℒ w → A × φ ℒ w)
--        _∪_ : Gram φ → Gram ψ → Gram (λ ℒ w → φ ℒ w ⊎ ψ ℒ w)
--        _∙_ : Gram φ → Gram ψ → Gram (λ ℒ w → ∃ λ u → ∃ λ v → (w ≡ u ++ v) × φ ℒ u × ψ ℒ v)
--        I : Gram (λ ℒ w → ℒ w)
--        μ : Gram 
--        _◃_ : (∀{ℒ w} → φ ℒ w ↔ ψ ℒ w) → Gram ψ → Gram φ

data Gram (v : Set₁) : Set₂ where
    ∅ : Gram v
    ε : Gram v
    ‵_ : (c : Tok) → Gram v
    _·_ : Dec A → Gram v → Gram v
    _∪_ : Gram v → Gram v → Gram v
    _∙_ : Gram v → Gram v → Gram v
    I : v → Gram v
    μ : (v → Gram v) → Gram v
...
-- δ′ : Gram φ → (c : Tok) → Gram (λ ℒ → δ c (φ ℒ))
-- δ′ ∅ c = {!!}
-- δ′ ε c = {!!}
-- δ′ (‵ c₁) c = {!!}
-- δ′ (x · G) c = {!!}
...
-- expr : Gram
-- expr = let open T in (‵ x) ∪ I ∙ (‵ +) ∙ I
-- 
-- -- for nested mus: indexed fixed points
-- 
-- setOf : Dec A → Set
-- setOf {A = A} _ = A
-- 
...
-- ⟦ ε ⟧ _ w = w ≡ []
-- ⟦ ‵ c ⟧ _ w = w ≡ (c ∷ [])
-- ⟦ A · G ⟧ L w = setOf A × ⟦ G ⟧ L w
-- ⟦ G ∪ G₁ ⟧ L w = ⟦ G ⟧ L w ⊎ ⟦ G₁ ⟧ L w
-- ⟦ G ∙ G₁ ⟧ L w = ∃ λ u → ∃ λ v → (w ≡ u ++ v) × ⟦ G ⟧ L u × ⟦ G₁ ⟧ L v
-- ⟦ I ⟧ L w = L w
-- 
-- variable L : Lang
-- 
...
-- map? = {!!}
-- 
-- ν′ : (G : Gram) → Dec (ν L) → Dec (ν (⟦ G ⟧ L))
-- ν′ ∅ νL = no (λ z → z)
-- ν′ ε νL = yes refl
-- ν′ (‵ c) νL = no (λ ())
-- ν′ (A · G) νL = A ×? ν′ G νL
-- ν′ (G ∪ G₁) νL = ν′ G νL ⊎? ν′ G₁ νL
-- ν′ (G ∙ G₁) νL = map? (λ (x , y) → [] , [] , refl , x , y) (λ where ([] , [] , refl , x , y) → x , y) (ν′ G νL ×? ν′ G₁ νL)
-- ν′ I νL = νL
-- 
-- ν′-correct : ν′ G ↔ ν ⟦ G ⟧
-- 
-- δ′ : Gram → Gram → (Tok → Gram)
-- δ′ ∅ _ _ = ∅
-- δ′ ε _ _ = ∅
-- δ′ (‵ c′) _ c with c ≟ c′
-- ... | yes _ = ε
-- ... | no _ = ∅
-- δ′ (x · G) G′ c = x · δ′ G G′ c
-- δ′ (G ∪ G₁) G′ c = δ′ G G′ c ∪ δ′ G₁ G′ c
-- δ′ (G ∙ G₁) G′ c = δ′ G G′ c ∙ G₁ ∪ (ν′ G {!!} · δ′ G₁ G′ c)
-- δ′ I G′ _ = G′
-- 
-- parse : (G : Gram) → (w : List Tok) → Dec (⟦ G ⟧ {!!} w)
-- parse G [] = ν′ G (no λ ())
-- parse G (c ∷ w) = map? {!!} {!!} (parse (δ′ G {!!} c) w)
-- 
-- 
...
-- -- 
-- --     data Fix (f : Gram n → Gram n) : Set where
-- --       In : ⟦ f (Fix f) ⟧ → Fix f
-- -- 
-- --     expr : Gram n
-- --     expr = expr' (▹ λ { .! → expr }) 
-- -- 
-- --     repeat : ℕ → Gram n → Gram n
-- --     repeat zero G = ε
...
-- -- -- --        (λ { zero → var zero ; (suc i) → subst (DecGram (suc m)) (sym (lookup-map _ Γδ′ i)) (renameDG suc (Γδ? i)) })
-- -- -- --        (λ { zero → renameDG suc (substDG σ (μ G) σ?) ; (suc i) → subst (DecGram (suc m)) (sym (lookup-map _ σ i)) (renameDG suc (σ? i)) })
-- -- -- --        G
-- -- -- --        c)
-- -- -- -- 
-- -- -- -- δ?₀ G c = δ? {Γ′ = []} [] (λ ()) (λ ()) (λ ()) G c
